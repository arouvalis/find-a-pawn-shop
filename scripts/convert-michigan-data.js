...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260315172843s97.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315172914s15.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315174701s8b.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-michigan.json");
...
// Filter to Michigan only, then filter to pawn shops, then map
const michiganRows = deduped.filter(isMichiganRow);
console.log(`Michigan rows: ${michiganRows.length}`);

const records = michiganRows.filter(isPawnShop).map((row) => {
