...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409222853s25.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-wyoming.json");
...
// Filter to Wyoming + OPERATIONAL only, then filter to pawn shops, then map
const wyRows = deduped.filter(isWyomingRow).filter(isOperational);
console.log(`Wyoming OPERATIONAL rows: ${wyRows.length}`);

const records = wyRows.filter(isPawnShop).map((row) => {
