...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406225504s5a.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406225532s2f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-new-mexico.json");
...
// Filter to New Mexico + OPERATIONAL only, then filter to pawn shops, then map
const nmRows = deduped.filter(isNewMexicoRow).filter(isOperational);
console.log(`New Mexico OPERATIONAL rows: ${nmRows.length}`);

const records = nmRows.filter(isPawnShop).map((row) => {
