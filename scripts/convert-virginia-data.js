...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260316212547s3b.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316212622s08.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-virginia.json");
...
function isVirginiaRow(row) {
  const state = String(row["state"] ?? row["State"] ?? "").trim().toUpperCase();
  return state === "VA" || state === "VIRGINIA";
}
...
// Filter to Virginia only, then filter to pawn shops, then map
const vaRows = deduped.filter(isVirginiaRow);
console.log(`Virginia rows: ${vaRows.length}`);

const records = vaRows.filter(isPawnShop).map((row) => {
