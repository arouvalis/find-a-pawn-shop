...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409212036s16 (1).xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-maine.json");
...
const meRows = deduped.filter(isMaineRow).filter(isOperational);
console.log(`Maine OPERATIONAL rows: ${meRows.length}`);

const records = meRows.filter(isPawnShop).map((row) => {
