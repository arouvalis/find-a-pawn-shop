...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409222944s82.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-south-dakota.json");
...
// Filter to South Dakota + OPERATIONAL only, then filter to pawn shops, then map
const sdRows = deduped.filter(isSouthDakotaRow).filter(isOperational);
console.log(`South Dakota OPERATIONAL rows: ${sdRows.length}`);

const records = sdRows.filter(isPawnShop).map((row) => {
