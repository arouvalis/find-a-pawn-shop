...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406231143s40.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406231207s06.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-delaware.json");
...
// Filter to Delaware + OPERATIONAL only, then filter to pawn shops, then map
const deRows = deduped.filter(isDelawareRow).filter(isOperational);
console.log(`Delaware OPERATIONAL rows: ${deRows.length}`);

const records = deRows.filter(isPawnShop).map((row) => {
