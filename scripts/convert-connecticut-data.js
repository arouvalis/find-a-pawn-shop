...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406222857s91.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406222922s8b.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-connecticut.json");
...
// Filter to Connecticut + OPERATIONAL only, then filter to pawn shops, then map
const ctRows = deduped.filter(isConnecticutRow).filter(isOperational);
console.log(`Connecticut OPERATIONAL rows: ${ctRows.length}`);

const records = ctRows.filter(isPawnShop).map((row) => {
