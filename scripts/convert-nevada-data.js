...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260315224120s8c.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315224143s05.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316185110s4f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-nevada.json");
...
// Filter to Nevada only, then filter to pawn shops, then map
const nevadaRows = deduped.filter(isNevadaRow);
console.log(`Nevada rows: ${nevadaRows.length}`);

const records = nevadaRows.filter(isPawnShop).map((row) => {
