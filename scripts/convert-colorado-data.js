...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260315223944s69.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315224011s9d.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-colorado.json");
...
function isColoradoRow(row) {
  const state = String(row["state"] ?? row["State"] ?? "").trim().toUpperCase();
  return state === "CO" || state === "COLORADO";
}
...
// Filter to Colorado only, then filter to pawn shops, then map
const coloradoRows = deduped.filter(isColoradoRow);
console.log(`Colorado rows: ${coloradoRows.length}`);

const records = coloradoRows.filter(isPawnShop).map((row) => {
