...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260318213109s9c.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260318213130s48.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-alabama.json");
...
// Filter to Alabama only, then filter to pawn shops, then map
const alRows = deduped.filter(isAlabamaRow);
console.log(`Alabama rows: ${alRows.length}`);

const records = alRows.filter(isPawnShop).map((row) => {
