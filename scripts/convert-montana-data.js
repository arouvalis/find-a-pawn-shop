...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260410155446s4c.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-montana.json");
...
const mtRows = deduped.filter(isMontanaRow).filter(isOperational);
console.log(`Montana OPERATIONAL rows: ${mtRows.length}`);

const records = mtRows.filter(isPawnShop).map((row) => {
