...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406222752s0d.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406222724s36.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-utah.json");
...
// Filter to Utah + OPERATIONAL only, then filter to pawn shops, then map
const utRows = deduped.filter(isUtahRow).filter(isOperational);
console.log(`Utah OPERATIONAL rows: ${utRows.length}`);

const records = utRows.filter(isPawnShop).map((row) => {
