...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230816s45.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230837s7b.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-massachusetts.json");
...
// Filter to Massachusetts + OPERATIONAL only, then filter to pawn shops, then map
const maRows = deduped.filter(isMassachusettsRow).filter(isOperational);
console.log(`Massachusetts OPERATIONAL rows: ${maRows.length}`);

const records = maRows.filter(isPawnShop).map((row) => {
