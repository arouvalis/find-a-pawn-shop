...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260319051606s3a.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260319051626s0d.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-arkansas.json");
...
// Filter to Arkansas only, then filter to pawn shops, then map
const arRows = deduped.filter(isArkansasRow);
console.log(`Arkansas rows: ${arRows.length}`);

const records = arRows.filter(isPawnShop).map((row) => {
