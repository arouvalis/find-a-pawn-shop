...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260316212408s86.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316212434s16.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-indiana.json");
...
// Filter to Indiana only, then filter to pawn shops, then map
const inRows = deduped.filter(isIndianaRow);
console.log(`Indiana rows: ${inRows.length}`);

const records = inRows.filter(isPawnShop).map((row) => {
