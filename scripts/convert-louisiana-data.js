...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260316225153s02.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316225214s94.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316230724s92.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-louisiana.json");
...
function isLouisianaRow(row) {
  const state = String(row["state"] ?? row["State"] ?? "").trim().toUpperCase();
  return state === "LA" || state === "LOUISIANA";
}
...
// Filter to Louisiana only, then filter to pawn shops, then map
const laRows = deduped.filter(isLouisianaRow);
console.log(`Louisiana rows: ${laRows.length}`);

const records = laRows.filter(isPawnShop).map((row) => {
