...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409152454s78.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409152512s4e.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-nebraska.json");
...
// Filter to Nebraska + OPERATIONAL only, then filter to pawn shops, then map
const neRows = deduped.filter(isNebraskaRow).filter(isOperational);
console.log(`Nebraska OPERATIONAL rows: ${neRows.length}`);

const records = neRows.filter(isPawnShop).map((row) => {
