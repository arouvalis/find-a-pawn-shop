...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409211947s75.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-new-hampshire.json");
...
// Filter to New Hampshire + OPERATIONAL only, then filter to pawn shops, then map
const nhRows = deduped.filter(isNewHampshireRow).filter(isOperational);
console.log(`New Hampshire OPERATIONAL rows: ${nhRows.length}`);

const records = nhRows.filter(isPawnShop).map((row) => {
