...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260410161846s4e.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-alaska.json");
...
const akRows = deduped.filter(isAlaskaRow).filter(isOperational);
console.log(`Alaska OPERATIONAL rows: ${akRows.length}`);

const records = akRows.filter(isPawnShop).map((row) => {
