...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406225716s47.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406225748s0f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-iowa.json");
...
// Filter to Iowa + OPERATIONAL only, then filter to pawn shops, then map
const iaRows = deduped.filter(isIowaRow).filter(isOperational);
console.log(`Iowa OPERATIONAL rows: ${iaRows.length}`);

const records = iaRows.filter(isPawnShop).map((row) => {
