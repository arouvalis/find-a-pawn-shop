...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260316184316s92.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316184342s58.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-tennessee.json");
...
// Filter to Tennessee only, then filter to pawn shops, then map
const tnRows = deduped.filter(isTennesseeRow);
console.log(`Tennessee rows: ${tnRows.length}`);

const records = tnRows.filter(isPawnShop).map((row) => {
