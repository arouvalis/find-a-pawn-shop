...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406230403s05.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406230433s70.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-west-virginia.json");
...
// Filter to West Virginia + OPERATIONAL only, then filter to pawn shops, then map
const wvRows = deduped.filter(isWestVirginiaRow).filter(isOperational);
console.log(`West Virginia OPERATIONAL rows: ${wvRows.length}`);

const records = wvRows.filter(isPawnShop).map((row) => {
