...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409223254s8c.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-north-dakota.json");
...
// Filter to North Dakota + OPERATIONAL only, then filter to pawn shops, then map
const ndRows = deduped.filter(isNorthDakotaRow).filter(isOperational);
console.log(`North Dakota OPERATIONAL rows: ${ndRows.length}`);

const records = ndRows.filter(isPawnShop).map((row) => {
