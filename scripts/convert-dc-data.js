...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260410161905s6f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-dc.json");
...
function isDCRow(row) {
  ...
  return (
    ...
    state.toUpperCase() === "DISTRICT OF COLUMBIA" ||
...
const dcRows = deduped.filter(isDCRow).filter(isOperational);
console.log(`DC OPERATIONAL rows: ${dcRows.length}`);

const records = dcRows.filter(isPawnShop).map((row) => {
