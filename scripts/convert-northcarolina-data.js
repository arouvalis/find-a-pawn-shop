...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260315180025s88.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315180046s37.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-northcarolina.json");
...
function isNorthCarolinaRow(row) {
  ...
  return state === "NC" || state === "NORTH CAROLINA";
}
...
// Filter to North Carolina only, then filter to pawn shops, then map
const ncRows = deduped.filter(isNorthCarolinaRow);
console.log(`North Carolina rows: ${ncRows.length}`);

const records = ncRows.filter(isPawnShop).map((row) => {
