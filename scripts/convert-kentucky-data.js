...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260318214110s54.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260318214142s29.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-kentucky.json");
...
function isKentuckyRow(row) {
  const state = String(row["state"] ?? row["State"] ?? "").trim().toUpperCase();
  return state === "KY" || state === "KENTUCKY";
}
...
// Filter to Kentucky only, then filter to pawn shops, then map
const kyRows = deduped.filter(isKentuckyRow);
console.log(`Kentucky rows: ${kyRows.length}`);

const records = kyRows.filter(isPawnShop).map((row) => {
