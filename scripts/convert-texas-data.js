...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260312215940s81.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260312220202s5e.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-texas.json");
...
const deduped = allRows.filter((row) => {
  ...
  if (!id) return true; // keep rows without a place_id
...
// Filter and map
const records = deduped.filter(isPawnShop).map((row) => {
