...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409152218s0c.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409152300s2b.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-mississippi.json");
...
// Filter to Mississippi + OPERATIONAL only, then filter to pawn shops, then map
const msRows = deduped.filter(isMississippiRow).filter(isOperational);
console.log(`Mississippi OPERATIONAL rows: ${msRows.length}`);

const records = msRows.filter(isPawnShop).map((row) => {
