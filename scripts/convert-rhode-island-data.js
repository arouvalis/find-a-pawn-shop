...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260410160227s40.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-rhode-island.json");
...
const riRows = deduped.filter(isRhodeIslandRow).filter(isOperational);
console.log(`Rhode Island OPERATIONAL rows: ${riRows.length}`);

const records = riRows.filter(isPawnShop).map((row) => {
