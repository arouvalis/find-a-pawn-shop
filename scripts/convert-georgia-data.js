...
// Filter to Georgia only, then filter to pawn shops, then map
const georgiaRows = deduped.filter(isGeorgiaRow);
console.log(`Georgia rows: ${georgiaRows.length}`);

const records = georgiaRows.filter(isPawnShop).map((row) => {
