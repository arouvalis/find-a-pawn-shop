...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230557s00.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230616s1d.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-new-jersey.json");
...
// Filter to New Jersey + OPERATIONAL only, then filter to pawn shops, then map
const njRows = deduped.filter(isNewJerseyRow).filter(isOperational);
console.log(`New Jersey OPERATIONAL rows: ${njRows.length}`);

const records = njRows.filter(isPawnShop).map((row) => {
