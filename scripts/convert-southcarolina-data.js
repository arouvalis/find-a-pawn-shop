...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260318213246s76.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260318213305s68.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-southcarolina.json");
...
function isSouthCarolinaRow(row) {
...
// Filter to South Carolina only, then filter to pawn shops, then map
const scRows = deduped.filter(isSouthCarolinaRow);
console.log(`South Carolina rows: ${scRows.length}`);

const records = scRows.filter(isPawnShop).map((row) => {
