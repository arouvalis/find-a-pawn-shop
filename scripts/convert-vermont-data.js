...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409212101s0d.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-vermont.json");
...
// Filter to Vermont + OPERATIONAL only, then filter to pawn shops, then map
const vtRows = deduped.filter(isVermontRow).filter(isOperational);
console.log(`Vermont OPERATIONAL rows: ${vtRows.length}`);

const records = vtRows.filter(isPawnShop).map((row) => {
