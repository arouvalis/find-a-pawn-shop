...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260406230023s94.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260406230058s00.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-kansas.json");
...
// Filter to Kansas + OPERATIONAL only, then filter to pawn shops, then map
const ksRows = deduped.filter(isKansasRow).filter(isOperational);
console.log(`Kansas OPERATIONAL rows: ${ksRows.length}`);

const records = ksRows.filter(isPawnShop).map((row) => {
