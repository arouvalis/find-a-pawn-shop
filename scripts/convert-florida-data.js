...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260313181441s46.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260313181516s8c.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-florida.json");
...
// Filter to Florida only, then filter to pawn shops, then map
const floridaRows = deduped.filter(isFloridaRow);
console.log(`Florida rows: ${floridaRows.length}`);

const records = floridaRows.filter(isPawnShop).map((row) => {
