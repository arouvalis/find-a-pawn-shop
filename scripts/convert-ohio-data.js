...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260314193558s15.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260314193649s3b.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315171256s3f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-ohio.json");
...
// Filter to Ohio only, then filter to pawn shops, then map
const ohioRows = deduped.filter(isOhioRow);
console.log(`Ohio rows: ${ohioRows.length}`);

const records = ohioRows.filter(isPawnShop).map((row) => {
