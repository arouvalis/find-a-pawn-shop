...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260314185017s84.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260314185041s75.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260314191033s84.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-arizona.json");
...
// Filter to Arizona only, then filter to pawn shops, then map
const arizonaRows = deduped.filter(isArizonaRow);
console.log(`Arizona rows: ${arizonaRows.length}`);

const records = arizonaRows.filter(isPawnShop).map((row) => {
