...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260314162315s27.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260314162353s5a.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-newyork.json");
...
// Filter to New York only, then filter to pawn shops, then map
const nyRows = deduped.filter(isNewYorkRow);
console.log(`New York rows: ${nyRows.length}`);

const records = nyRows.filter(isPawnShop).map((row) => {
