...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260315223531s9a.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315223556s18.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316161835s01.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-washington.json");
...
// Filter to Washington only, then filter to pawn shops, then map
const waRows = deduped.filter(isWashingtonRow);
console.log(`Washington rows: ${waRows.length}`);

const records = waRows.filter(isPawnShop).map((row) => {
