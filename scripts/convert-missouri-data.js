...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260316184515s4a.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260316184537s9f.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-missouri.json");
...
function isMissouriRow(row) {
  const state = String(row["state"] ?? row["State"] ?? "").trim().toUpperCase();
  return state === "MO" || state === "MISSOURI";
}
...
// Filter to Missouri only, then filter to pawn shops, then map
const moRows = deduped.filter(isMissouriRow);
console.log(`Missouri rows: ${moRows.length}`);

const records = moRows.filter(isPawnShop).map((row) => {
