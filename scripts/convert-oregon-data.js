...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230635s8e.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409230655s18.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-oregon.json");
...
// Filter to Oregon + OPERATIONAL only, then filter to pawn shops, then map
const orRows = deduped.filter(isOregonRow).filter(isOperational);
console.log(`Oregon OPERATIONAL rows: ${orRows.length}`);

const records = orRows.filter(isPawnShop).map((row) => {
