...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260409153043s1b.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260409153106s60.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-idaho.json");
...
// Filter to Idaho + OPERATIONAL only, then filter to pawn shops, then map
const idRows = deduped.filter(isIdahoRow).filter(isOperational);
console.log(`Idaho OPERATIONAL rows: ${idRows.length}`);

const records = idRows.filter(isPawnShop).map((row) => {
