...
const INPUTS = [
  path.join(process.env.HOME, "Downloads/Outscraper-20260313221825xl28.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260313221859xl20.xlsx"),
  path.join(process.env.HOME, "Downloads/Outscraper-20260315165846s8b.xlsx"),
];
const OUTPUT = path.join(__dirname, "../data/pawn-shops-california.json");
...
// Filter to California only, then filter to pawn shops, then map
const californiaRows = deduped.filter(isCaliforniaRow);
console.log(`California rows: ${californiaRows.length}`);

const records = californiaRows.filter(isPawnShop).map((row) => {
