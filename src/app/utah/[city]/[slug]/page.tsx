...
export async function generateStaticParams() {
  return getAllUtahShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getUtahShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, Utah — FindAPawnShop.com`,
    description: buildUtahSeoDescription(shop),
  };
}

export default async function UtahListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getUtahShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildUtahSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/utah/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
