...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const vermontCities = getVermontCities();
  const vermontShops = getAllVermontShops();
  ...
  const alaskaCityPages: MetadataRoute.Sitemap = alaskaCities.map(({ citySlug }) => ({
    url: `${BASE_URL}/alaska/${citySlug}`,
    changeFrequency: "monthly",
    priority: 0.8,
  }));

  const alaskaListingPages: MetadataRoute.Sitemap = alaskaShops.map((shop) => ({
    url: `${BASE_URL}/alaska/${shop.citySlug}/${shop.slug}`,
    changeFrequency: "monthly",
    priority: 0.7,
  }));

  const dcCityPages: MetadataRoute.Sitemap = dcCities.map(({ citySlug }) => ({
    url: `${BASE_URL}/dc/${citySlug}`,
    changeFrequency: "monthly",
    priority: 0.8,
  }));

  const dcListingPages: MetadataRoute.Sitemap = dcShops.map((shop) => ({
    url: `${BASE_URL}/dc/${shop.citySlug}/${shop.slug}`,
    changeFrequency: "monthly",
    priority: 0.7,
  }));
  ...
  const vermontCityPages: MetadataRoute.Sitemap = vermontCities.map(({ citySlug }) => ({
    url: `${BASE_URL}/vermont/${citySlug}`,
    changeFrequency: "monthly",
    priority: 0.8,
  }));

  const vermontListingPages: MetadataRoute.Sitemap = vermontShops.map((shop) => ({
    url: `${BASE_URL}/vermont/${shop.citySlug}/${shop.slug}`,
    changeFrequency: "monthly",
    priority: 0.7,
  }));
