...
import {
  getAllNorthCarolinaShops,
  getNorthCarolinaShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildNorthCarolinaSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllNorthCarolinaShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getNorthCarolinaShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, North Carolina — FindAPawnShop.com`,
    description: buildNorthCarolinaSeoDescription(shop),
  };
}

export default async function NorthCarolinaListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getNorthCarolinaShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildNorthCarolinaSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/north-carolina/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
