...
import { getSouthDakotaCities, getSouthDakotaShopsByCity, formatAddress } from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getSouthDakotaCities().map(({ citySlug }) => ({ city: citySlug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { city: citySlug } = await params;
  const shops = getSouthDakotaShopsByCity(citySlug);
  if (!shops.length) return {};
  const cityName = shops[0].city;
  return {
    title: `Pawn Shops in ${cityName}, South Dakota — FindAPawnShop.com`,
    description: `Find the best pawn shops in ${cityName}, South Dakota. Browse ${shops.length} verified listings with addresses, hours, phone numbers, and ratings. Compare pawn shops near you.`,
  };
}

export default async function SouthDakotaCityPage({ params }: Props) {
  const { city: citySlug } = await params;
  const shops = getSouthDakotaShopsByCity(citySlug);
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: `Pawn Shops in ${cityName}, South Dakota`,
    description: `Directory of pawn shops in ${cityName}, South Dakota`,
    numberOfItems: shops.length,
    itemListElement: sorted.map((shop, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: shop.name,
      url: `https://www.findapawnshop.com/south-dakota/${citySlug}/${shop.slug}`,
    })),
  };
    ...
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/south-dakota" className="hover:text-amber-600">South Dakota</Link>
          <span>/</span>
          <span className="text-gray-900">{cityName}</span>
        </nav>

        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          Pawn Shops in {cityName}, South Dakota
        </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {sorted.map((shop) => (
            <Link
              key={shop.slug}
              href={`/south-dakota/${citySlug}/${shop.slug}`}
