...
import {
  getAllSouthDakotaShops,
  getSouthDakotaShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildSouthDakotaSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllSouthDakotaShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getSouthDakotaShopBySlug(city, slug);
  if (!shop) return {};
  return {
...
export default async function SouthDakotaListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getSouthDakotaShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildSouthDakotaSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/south-dakota/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
