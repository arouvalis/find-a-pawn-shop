...
export async function generateStaticParams() {
  return getAllIdahoShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getIdahoShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, Idaho — FindAPawnShop.com`,
    description: buildIdahoSeoDescription(shop),
  };
}

export default async function IdahoListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getIdahoShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildIdahoSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/idaho/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
