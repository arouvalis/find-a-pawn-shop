...
import { getOhioCities, getAllOhioShops } from "@/lib/pawnShops";

export function generateMetadata(): Metadata {
  const cities = getOhioCities().sort((a, b) => b.count - a.count);
  const total = getAllOhioShops().length;
  const top = cities.slice(0, 3).map((c) => c.city);
  return {
    title: "Pawn Shops in Ohio — FindAPawnShop.com",
    description: `Browse ${total} verified pawn shops across Ohio — find addresses, hours, phone numbers, and ratings for pawn shops in ${top.join(", ")} and more.`,
  };
}

export default function OhioPage() {
  const cities = getOhioCities();
  const total = getAllOhioShops().length;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-2">Pawn Shops in Ohio</h1>
      ...
      <div style={{ backgroundColor: "#1a2744" }} className="mt-10 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-sm text-white">
          Own a pawn shop in <span className="font-semibold text-amber-400">Ohio</span>? Get featured placement for $29/month.
        </p>
        <Link
          href="/claim-listing"
