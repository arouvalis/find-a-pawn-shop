...
import {
  getAllNorthDakotaShops,
  getNorthDakotaShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildNorthDakotaSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllNorthDakotaShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getNorthDakotaShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, North Dakota — FindAPawnShop.com`,
    description: buildNorthDakotaSeoDescription(shop),
  };
}

export default async function NorthDakotaListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getNorthDakotaShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildNorthDakotaSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/north-dakota/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
        ...
        <div className="border border-gray-200 rounded-xl p-6 space-y-5 mb-8 mt-8">
          ...
          {shop.website && (
            <div>
              ...
              <a href={shop.website} target="_blank" rel="noopener noreferrer" className="text-amber-600 hover:underline break-all">
                {shop.website}
              </a>
            </div>
          )}
