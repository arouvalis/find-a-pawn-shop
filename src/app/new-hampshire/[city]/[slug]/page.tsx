...
import {
  getAllNewHampshireShops,
  getNewHampshireShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildNewHampshireSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllNewHampshireShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getNewHampshireShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, New Hampshire — FindAPawnShop.com`,
    description: buildNewHampshireSeoDescription(shop),
  };
}

export default async function NewHampshireListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getNewHampshireShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildNewHampshireSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/new-hampshire/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
