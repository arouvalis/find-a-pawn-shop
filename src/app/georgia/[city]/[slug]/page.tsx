...
import {
  getAllGeorgiaShops,
  getGeorgiaShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildGeorgiaSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllGeorgiaShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getGeorgiaShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, Georgia — FindAPawnShop.com`,
    description: buildGeorgiaSeoDescription(shop),
  };
}

export default async function GeorgiaListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getGeorgiaShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildGeorgiaSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/georgia/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
