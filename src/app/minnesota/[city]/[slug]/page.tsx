...
import {
  getAllMinnesotaShops,
  getMinnesotaShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildMinnesotaSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllMinnesotaShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getMinnesotaShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, Minnesota — FindAPawnShop.com`,
    description: buildMinnesotaSeoDescription(shop),
  };
}

export default async function MinnesotaListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getMinnesotaShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildMinnesotaSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/minnesota/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
