...
import {
  getAllConnecticutShops,
  getConnecticutShopBySlug,
  parseHours,
  formatAddress,
  toOpeningHoursSchema,
  buildConnecticutSeoDescription,
} from "@/lib/pawnShops";
...
export async function generateStaticParams() {
  return getAllConnecticutShops().map((shop) => ({
    city: shop.citySlug,
    slug: shop.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const shop = getConnecticutShopBySlug(city, slug);
  if (!shop) return {};
  return {
    title: `${shop.name} — ${shop.city}, Connecticut — FindAPawnShop.com`,
    description: buildConnecticutSeoDescription(shop),
  };
}

export default async function ConnecticutListingPage({ params }: Props) {
  const { city: citySlug, slug } = await params;
  const shop = getConnecticutShopBySlug(citySlug, slug);
  if (!shop) notFound();

  const hours = parseHours(shop.hours);
  const address = formatAddress(shop);
  const openingHours = toOpeningHoursSchema(shop.hours);
  const seoDescription = buildConnecticutSeoDescription(shop);
    ...
    <>
      ...
      <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/connecticut/${citySlug}`} className="hover:text-amber-600">{shop.city}</Link>
          <span>/</span>
