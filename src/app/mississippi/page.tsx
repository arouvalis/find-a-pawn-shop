...
import { getMississippiCities, getAllMississippiShops } from "@/lib/pawnShops";

export function generateMetadata(): Metadata {
  const cities = getMississippiCities().sort((a, b) => b.count - a.count);
  const total = getAllMississippiShops().length;
  const top = cities.slice(0, 3).map((c) => c.city);
  return {
    title: "Pawn Shops in Mississippi — FindAPawnShop.com",
    description: `Browse ${total} verified pawn shops across Mississippi — find addresses, hours, phone numbers, and ratings for pawn shops in ${top.join(", ")} and more.`,
  };
}

export default function MississippiPage() {
  const cities = getMississippiCities();
  const total = getAllMississippiShops().length;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-gray-900">Mississippi</span>
      </nav>

      <h1 className="text-3xl font-bold text-gray-900 mb-2">Pawn Shops in Mississippi</h1>
      ...
      <div style={{ backgroundColor: "#1a2744" }} className="mt-10 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-sm text-white">
          Own a pawn shop in <span className="font-semibold text-amber-400">Mississippi</span>? Get featured placement for $29/month.
        </p>
        <Link
          href="/claim-listing"
