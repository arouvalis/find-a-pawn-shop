...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Find the best pawn shops in ${cityName}, Illinois. Browse ${shops.length} verified listings with addresses, hours, phone numbers, and ratings. Compare pawn shops near you.`,
  };
}
...
export default async function CityPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: `Pawn Shops in ${cityName}, Illinois`,
    description: `Directory of pawn shops in ${cityName}, Illinois`,
    numberOfItems: shops.length,
    itemListElement: sorted.map((shop, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: shop.name,
      url: `https://www.findapawnshop.com/illinois/${citySlug}/${shop.slug}`,
    })),
  };
