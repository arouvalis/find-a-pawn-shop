...
import { getCities, getAllShops } from "@/lib/pawnShops";

export function generateMetadata(): Metadata {
  const cities = getCities().sort((a, b) => b.count - a.count);
  const total = getAllShops().length;
  const top = cities.slice(0, 3).map((c) => c.city);
  return {
    title: "Pawn Shops in Illinois — FindAPawnShop.com",
    description: `Browse ${total} verified pawn shops across Illinois — find addresses, hours, phone numbers, and ratings for pawn shops in ${top.join(", ")} and more.`,
  };
}
...
export default function IllinoisPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-2">Pawn Shops in Illinois</h1>
      ...
      <div style={{ backgroundColor: "#1a2744" }} className="mt-10 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-sm text-white">
          Own a pawn shop in <span className="font-semibold text-amber-400">Illinois</span>? Get featured placement for $29/month.
        </p>
        <Link
          href="/claim-listing"
